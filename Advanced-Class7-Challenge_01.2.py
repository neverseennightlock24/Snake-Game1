...
# This version allows for precise Apple spawning at the cost of Snake overlap
...
def spawn_apple():
    apple_x = random.randint(2, 37) * 20
    apple_y = random.randint(2, 37) * 20
    return pygame.Rect(apple_x, apple_y, 20, 20)

apple_rect = spawn_apple()
...
def restart_game():
    global snake_x, snake_y, snake_rect, snake_segments, snake_dx, snake_dy, apple_rect, score, game_over
    snake_x = 400
    snake_y = 400
    snake_rect = pygame.Rect(snake_x, snake_y, 20, 20)
    snake_segments = [snake_rect]
...
def main_game_loop():
    ...
    text_rect2 = text2.get_rect(center = (400, 450))
